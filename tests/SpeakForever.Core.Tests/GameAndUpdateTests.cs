...
public sealed class GameAndUpdateTests : IDisposable
{
    ...
    {
        const string Json = """{ "tag_name": "v1.2.0", "html_url": "https://github.com/o/r/releases/tag/v1.2.0", "draft": false, "prerelease": false, "assets": [] }""";
        var release = JsonSerializer.Deserialize(Json, ReleaseJson.Default.Release);
        Assert.Equal("https://github.com/o/r/releases/tag/v1.2.0", UpdateChecker.NewerThan(release, new Version(1, 0, 0))?.PageUrl);
    }
